...
//Updated by Celestialamber on 1/2023 to work with SNES graphics instead. Only png -> 4bpp has been changed
//as of now.
...
void ConvertPngToGba(char *inputPath, char *outputPath, struct PngToGbaOptions *options)
{
    ...
    WriteImage(outputPath, options->numTilesMode, options->numTiles, options->bitDepth, options->metatileWidth, options->metatileHeight, &image, !image.hasPalette, options->tileDirection);
...
void HandlePngToGbaCommand(char *inputPath, char *outputPath, int argc, char **argv)
{
    ...
    options.tilemapFilePath = NULL;
    options.isAffineMap = false;
    options.tileDirection = TILE_DIR_HORIZONTAL;
    ...
    for (int i = 3; i < argc; i++)
    {
        ...
        else if (strcmp(option, "-mheight") == 0)
        {
            ...
            options.tileDirection = TILE_DIR_VERTICAL;
        }
        else
        {
            FATAL_ERROR("Unrecognized option \"%s\".\n", option);
